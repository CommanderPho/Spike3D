...
class AxisItem(GraphicsWidget):
    ...
    def __init__(self, orientation, pen=None, linkView=None, parent=None, maxTickLength=-5, showValues=True):
        ...
        **Arguments:**
        ...
        self.style = {
            ...
            'tickFont': QtGui.QFont("Kristen ITC"),
        ...
        self.labelStyle = {}
        self.logMode = False
        self.tickFont = QtGui.QFont("Kristen ITC", pointSize = 10)
    ...
    def tickValues(self, minVal, maxVal, size):
        ...
        if self.linked_axis is None:
            tickLevels = self.tickSpacing(minVal, maxVal, size)
            self.ticked_to_linked = tickLevels
        else:
            tickLevels = self.linked_axis.ticked_to_linked
        #print("levels... ", self.orientation, tickLevels)
        allValues = np.array([])
    ...
    def generateDrawSpecs(self, p):
        ...
        #print(self.parent.mapToScene(Point(0, 0)))
        #print("type", type(self.parent))
        #print("-------------", self.linkedView().mapFromView(Point(0, 0)))
        ...
        if lengthInPixels == 0:
            return
        #print(self.orientation, "lengthInPixels", lengthInPixels)
        ...
        self.tickPositions = [] # remembers positions of previously drawn ticks
        
        ## compute coordinates to draw ticks
        ## draw three different intervals, long ticks first
        tickSpecs = []
        for i in range(len(tickLevels)):
            self.tickPositions.append([])
            ...
            for v in ticks:
                ...
                if x < xMin or x > xMax:  ## last check to make sure no out-of-bounds ticks are drawn
                    self.tickPositions[i].append(None)
                    continue
                self.tickPositions[i].append(x)
        ...
        if self.style['stopAxisAtTick'][0] is True:
            stop = max(span[0].y(), min(map(min, self.tickPositions)))
            if axis == 0:
                span[0].setY(stop)
            else:
                span[0].setX(stop)
        ...
        if self.style['stopAxisAtTick'][1] is True:
            stop = min(span[1].y(), max(map(max, self.tickPositions)))
            if axis == 0:
                span[1].setY(stop)
            else:
                span[1].setX(stop)
        ...
        #print("self.parent.mapFromView(Point(0, 0)", self.parent.mapFromView(Point(0, 0)))
        if self.orientation == "left":
            left_axis_x = self.parent.mapFromView(Point(0, 0)).x()
        else:
            bottom_axis_y = self.geometry().y() - self.parent.mapFromView(Point(0, 0)).y()
        ...
        for i in range(min(len(tickLevels), self.style['maxTextLevel']+1)):
            ...
            for j in range(len(strings)):
                if self.tickPositions[i][j] is None:
                    strings[j] = None
            ...
            for j in range(len(strings)):
                ...
                offset = max(0,self.style['tickLength']) + textOffset - 1

                if self.orientation == 'left':
                    textFlags = QtCore.Qt.TextDontClip|QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter
                    rect = QtCore.QRectF(left_axis_x + 80- width - offset, x-(height/2), width, height)
                    if s == '0':
                        rect = QtCore.QRectF(left_axis_x + 80- width - offset, x + 2, width, height)
                elif self.orientation == 'bottom':
                    textFlags = QtCore.Qt.TextDontClip|QtCore.Qt.AlignCenter|QtCore.Qt.AlignTop
                    rect = QtCore.QRectF(x-width/2., -bottom_axis_y + offset + 1, width, height)
    ...
    def drawPicture(self, p, axisSpec, tickSpecs, textSpecs):
        ...
        for rect, flags, text in textSpecs:
            ...
            if text == '0' and self.orientation == 'bottom':
                continue
            p.setPen(fn.mkPen(0, 0, 0, 170))
