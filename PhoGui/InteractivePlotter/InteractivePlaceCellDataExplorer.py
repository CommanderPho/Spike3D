...
# show_legend = True
	...
	@property
	def historical_main_spikes_mesh(self):
		return self.plots.get('historical_main_spikes_mesh', None)
	@property
	def recent_only_main_spikes_mesh(self):
		return self.plots.get('recent_only_main_spikes_mesh', None)
	
	@property
	def animal_location_trail_mesh(self):
		return self.plots.get('animal_location_trail_mesh', None)
	
	@property
	def animal_current_location_point_mesh(self):
		return self.plots.get('animal_current_location_point_mesh', None)

	def perform_plot_location_point(self, plot_name, curr_animal_point, render):
	...
	def on_programmatic_data_update(self, active_included_all_historical_indicies=None, active_included_recent_only_indicies=None, active_window_sample_indicies=None, curr_animal_point=None):
		...
		if active_included_all_historical_indicies is not None:
			...
			if historical_spikes_pc.n_points >= 1:
				self.plots['historical_main_spikes_mesh'] = self.p.add_mesh(historical_spikes_pc, name='historical_spikes_main', scalars='cellID', cmap=self.active_config.plotting_config.active_cells_listed_colormap, show_scalar_bar=False, lighting=True, render=False)
				needs_render = True
		...
		if active_included_recent_only_indicies is not None:
			...
			if recent_only_spikes_pc.n_points >= 1:
				self.plots['recent_only_main_spikes_mesh'] = self.p.add_mesh(recent_only_spikes_pc, name='recent_only_spikes_main', scalars='cellID', cmap=self.active_config.plotting_config.active_cells_listed_colormap, show_scalar_bar=False, lighting=False, render=False) # color='white'
				needs_render = True
		...
		if active_window_sample_indicies is not None:
			...
			self.plots['animal_location_trail_mesh'] = self.p.add_mesh(pc_positionTrail, name='animal_location_trail', ambient=0.6, opacity='linear_r', scalars='pho_fade_values', nan_opacity=0.0,
		...
		if curr_animal_point is not None:
			...
			pdata_current_point = pv.PolyData(curr_animal_point) # a mesh
			pc_current_point = pdata_current_point.glyph(scale=False, geom=animal_location_circle)
			self.plots['animal_current_location_point_mesh'] = self.p.add_mesh(pc_current_point, name='animal_location', color='green', ambient=0.6, opacity=0.5,
															show_edges=True, edge_color=[0.05, 0.8, 0.08], line_width=3.0, nan_opacity=0.0, render_lines_as_tubes=True,
